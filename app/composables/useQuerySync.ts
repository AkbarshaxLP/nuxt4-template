...
export function useQuerySync(objectToSync: IFilterObject, refreshCallback?: () => void) {
  ...
  function updateQuery(pagination?: boolean | undefined) {
    ...
    router
      .replace({
        query: pagination
          ? sanitizeQuery({ ...route.query, page: newState.page, page_size: newState.page_size })
  ...
  return { updateQuery, clearQuery, previousFilter, resetPagination }
}
