  ...
  try {
    ...
    for (const [path, pathItem] of Object.entries(swaggerSchema.paths)) {
      ...
      for (const method of ['get', 'post', 'put', 'patch', 'delete']) {
        ...
        methodName = methodName.replace(/[^a-zA-Z0-9_$]/g, '_').replace(/^[0-9]/, '_$&')
        ...
        const successResponse =
          Object.entries(operation.responses || {}).find(
            ([code]) => !excludeStatusCodeStarts.includes(String(code)[0])
          ) || []
        const responseContentTypes = successResponse[1]?.content
          ? Object.keys(successResponse[1].content)
          : []
        responseContentType =
          responseContentTypes.length > 0 ? responseContentTypes[0] : 'application/json'
        ...
        if (!hasPathParams && !hasQueryParams && !hasBody) {
          ...
          const pathWithParams = pathParams.reduce(
            (acc, param) => acc.replace(`{${param}}`, `\${${param}}`),
            path
          )
          ...
          if (hasBody) {
            ...
            if (hasQueryParams) {
              ...
              apiLine =
                `${methodName}: (${pathParamsStr ? pathParamsStr + ', ' : ''}${bodyParam}: ${bodyType}, query?: ${queryParamsType}) => ` +
              ...
              apiLine =
                `${methodName}: (${pathParamsStr ? pathParamsStr + ', ' : ''}${bodyParam}: ${bodyType}) => ` +
                `useApi<${responseType}>(\`${pathWithParams}\`, { ` +
                `method: '${method.toUpperCase()}', ` +
            ...
            const queryParamsType = operation.parameters?.some((p) => p.in === 'query')
              ...
              : 'Record<string, unknown>'
            apiLine =
              `${methodName}: (${pathParamsStr ? pathParamsStr + ', ' : ''}query?: ${queryParamsType}) => ` +
    ...
    for (const [groupName, methods] of Object.entries(groups)) {
      const groupNameFormat = groupName.replace(/[^a-zA-Z0-9_$]/g, '_').replace(/^[0-9]/, '_$&')
